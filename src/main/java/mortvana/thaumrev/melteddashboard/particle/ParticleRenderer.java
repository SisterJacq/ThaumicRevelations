...
public class ParticleRenderer {
	...
	public void renderParticles(RenderWorldLastEvent event) {
		if (worldRef.get() == null || particles.isEmpty()) {
			return;
		}
		...
		EntityLivingBase ent = mc.renderViewEntity;

		ParticleBase.interpPosX = ent.lastTickPosX + (ent.posX - ent.lastTickPosX) * (double) event.partialTicks;
		ParticleBase.interpPosY = ent.lastTickPosY + (ent.posY - ent.lastTickPosY) * (double) event.partialTicks;
		ParticleBase.interpPosZ = ent.lastTickPosZ + (ent.posZ - ent.lastTickPosZ) * (double) event.partialTicks;
